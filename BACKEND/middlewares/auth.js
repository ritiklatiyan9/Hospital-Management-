...
export const isAdminAuthenticated = catchAsyncErrors(async (req, res, next) => {
  ...
  if (req.user.role !== "Admin") {
    return next(
      new ErrorHandler(`${req.user.role} is not authorized for this role`, 403)
    );
  }
...
export const isPatientAuthenticated = catchAsyncErrors(async (req, res, next) => {
  const token = req.cookies.patientToken;  // Ensure token is present in cookies
  if (!token) {
    return next(new ErrorHandler("Token Patient not authenticated", 401));
  }

  const decoded = jwt.verify(token, process.env.JWT_SECRET_KEY);
  req.user = await User.findById(decoded.id); // Retrieve user from the token
